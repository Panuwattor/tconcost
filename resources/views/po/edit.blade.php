...
<form id="app" v-cloak action="/po/update" method="post" enctype="multipart/form-data" onsubmit="return confirm('ยืนยันแก้ไขการสั่งซื้อ ข้อมูลขอซื้อถูกต้อง ?')">
    @csrf
                    ...
                    <tbody>
                        <template v-for="(item, index) in arr">
                            <tr>
                                ...
                                <td>
                                    <span @click="remove(index)" style="cursor: pointer;" v-if="arr.length > 1 && !item.id" class="text-danger"><i class="fa fa-trash-o"></i> ลบ</span>
                                </td>
                            </tr>
                        </template>
                    </tbody>
