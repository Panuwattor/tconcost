...
@include('project.heade')
    <div class="card">
        ...
        <form action="/project/detail/budget/{{$project->id}}" method="post"  onSubmit="if(!confirm('ยืนยัน การทำรายการ หรือ ไม่ ?')){return false;}">
         @csrf
        <div class="row">
              ...
              <div class="card-body p-0">
                <table class="table table-sm table-bordered">
                  ...
                  @foreach($costplan->costPlanLists as $i => $costplanlist)
                    <tr>
                      <td><p class="mb-0 mt-2 mr-2">{{$costplanlist->code}}</p> </td>
                      <td><p class="mb-0 mt-2 mr-2">{{$costplanlist->name}}</p> </td>
                      <td>
                         <input class="form-control rounded-0 border-0 shadow-none placeholder-Desription ng-untouched ng-pristine ng-valid ng-star-inserted" name="descriptions[{{$costplanlist->id}}]" type="text" placeholder=" เพิ่มรายละเอียดรหัสต้นทุน ">
                      </td>
                      <td><input class="form-control rounded-0 text-right border-0 shadow-none inputNum" name="costs[{{$costplanlist->id}}]" type="number" step="any" placeholder="0.00"></td>
                    </tr>
                    @endforeach
                  </tbody>
                </table>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="info-box mb-3">
                            ...
                            <div class="info-box-content">
                                <h5>จำนวนเงิน</h5>
                                <input type="text" class="form-control is-valid" id="sumCost" name="sumCost" value="0" readonly>
                            </div>
